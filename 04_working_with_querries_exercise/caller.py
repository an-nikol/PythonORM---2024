...
from main_app.choices import OperatingSystemChoices

# Set up Django
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "orm_skeleton.settings")
django.setup()

from main_app.models import Laptop, ArtworkGallery, ChessPlayer, Meal, Dungeon, Workout
...
def bulk_create_arts(first_art: ArtworkGallery, second_art: ArtworkGallery):
    ...
    bulk_art = ArtworkGallery.objects.bulk_create(new_art)


def delete_negative_rated_arts():
    arts_to_be_deleted = ArtworkGallery.objects.filter(rating__lt=0)

    arts_to_be_deleted.delete()
...
def update_operation_systems():
    Laptop.objects.all().update(
        operation_system=Case(
            When(brand='Asus', then=Value(OperatingSystemChoices.WINDOWS)),
            When(brand='Apple', then=Value(OperatingSystemChoices.MACOS)),
            When(brand__in=('Dell', 'Acer'), then=Value(OperatingSystemChoices.LINUX)),
            When(brand='Lenovo', then=Value(OperatingSystemChoices.CHROME_OS))
        )
    )
...
def show_workouts() -> str:
    cali_crossfit_workouts = Workout.objects.filter(workout_type__in=('Calisthenics', 'CrossFit')).order_by('id')

    result = []

    for w in cali_crossfit_workouts:
